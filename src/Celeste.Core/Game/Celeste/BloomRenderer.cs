...
public class BloomRenderer : Renderer
{
	...
	public static readonly BlendState LegacyCutoutBlendstate = new BlendState
	{
	...
	public void Apply(VirtualRenderTarget target, Scene scene)
	{
		...
		if (Base < 1f)
		{
			...
			if (components2.Count > 0)
			{
				try
				{
					DrawCutoutMask(components2, camera, ResolveCutoutBlendState());
				}
				catch (Exception exception) when (GraphicsBlendCompatibility.TryEnableLegacyBlendFallback("BloomRenderer.CutoutBlendstate", exception))
				{
					DrawCutoutMask(components2, camera, LegacyCutoutBlendstate);
				}
			}
		}
	...
	private static BlendState ResolveCutoutBlendState()
	{
		return GraphicsBlendCompatibility.UseLegacyBlendStates ? LegacyCutoutBlendstate : CutoutBlendstate;
	}

	private static void DrawCutoutMask(List<Component> cutouts, Camera camera, BlendState blendState)
	{
		Draw.SpriteBatch.Begin(SpriteSortMode.Deferred, blendState, SamplerState.PointClamp, null, null, null, camera.Matrix);
		foreach (Component item in cutouts)
		{
			EffectCutout effectCutout = item as EffectCutout;
