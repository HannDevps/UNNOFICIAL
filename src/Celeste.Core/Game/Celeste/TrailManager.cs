...
public class TrailManager : Entity
{
	...
	private void BeforeRender()
	{
		...
		DrawMaskClearPass(ResolveMaskClearBlendState(), "TrailManager.BeforeRender.MaskClear");
	...
	private static BlendState ResolveMaskClearBlendState()
	{
		return GraphicsBlendCompatibility.UseLegacyBlendStates ? LightingRenderer.LegacyOccludeBlendState : LightingRenderer.OccludeBlendState;
	}

	private void DrawMaskClearPass(BlendState blendState, string source)
	{
		try
		{
			DrawMaskClearPassCore(blendState);
		}
		catch (Exception exception) when (GraphicsBlendCompatibility.TryEnableLegacyBlendFallback(source, exception))
		{
			DrawMaskClearPassCore(LightingRenderer.LegacyOccludeBlendState);
		}
	}

	private void DrawMaskClearPassCore(BlendState blendState)
	{
