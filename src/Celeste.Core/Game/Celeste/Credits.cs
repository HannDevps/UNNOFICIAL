...
public class Credits
{
	...
	private class Role : CreditNode
	{
		...
		{
			Font.DrawOutline(FontSize, Name, position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 1.8f * scale, NameColor, 2f, BorderColor);
			position.Y += (LineHeight * 1.8f + 8f) * scale;
			Font.DrawOutline(FontSize, Roles, position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 1f * scale, RolesColor, 2f, BorderColor);
		}
	...
	private class Team : CreditNode
	{
		...
		public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
		{
			Font.DrawOutline(FontSize, Name, position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 1.8f * scale, Role.NameColor, 2f, BorderColor);
			position.Y += (LineHeight * 1.8f + 8f) * scale;
			for (int i = 0; i < Members.Length; i++)
			{
				Font.DrawOutline(FontSize, Members[i], position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 1.4f * scale, TeamColor, 2f, BorderColor);
				position.Y += LineHeight * 1.4f * scale;
			}
			Font.DrawOutline(FontSize, Roles, position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 1f * scale, Role.RolesColor, 2f, BorderColor);
		}
	...
	private class Thanks : CreditNode
	{
		...
		public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
		{
			...
			Font.DrawOutline(FontSize, Title, position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 1.4f * scale, TitleColor, 2f, BorderColor);
			...
			for (int i = 0; i < Credits.Length; i++)
			{
				if (linkedImages[i] != null)
				{
					...
					mTexture.DrawJustified(position.FloorV2() + new Vector2(0f, -2f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(position.FloorV2() + new Vector2(0f, 2f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(position.FloorV2() + new Vector2(-2f, 0f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(position.FloorV2() + new Vector2(2f, 0f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(position.FloorV2(), new Vector2(alignment, 0f), CreditsColor, 1.15f * scale, 0f);
					position.Y += (float)mTexture.Height * 1.15f * scale;
				}
				else
				{
					Font.DrawOutline(FontSize, Credits[i], position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 1.15f * scale, CreditsColor, 2f, BorderColor);
	...
	private class MultiCredit : CreditNode
	{
		...
		public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
		{
			...
			for (int i = 0; i < Sections.Length; i++)
			{
				...
				_ = section.Subtitle;
				Font.DrawOutline(FontSize, section.Subtitle, position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 0.7f * scale, SubtitleColor, 2f, BorderColor);
				position.Y += (float)section.SubtitleLines * LineHeight * 0.7f * scale;
				for (int j = 0; j < section.Credits.Length; j++)
				{
					Font.DrawOutline(FontSize, section.Credits[j], position.FloorV2(), new Vector2(alignment, 0f), Vector2.One * 1.15f * scale, CreditsColor, 2f, BorderColor);
					position.Y += LineHeight * 1.15f * scale;
				}
	...
	private static List<CreditNode> CreateCredits(bool title, bool polaroids)
	{
		...
		{
			new Role("Hann Augusto", "Port criado Por Hann Augusto"),
	...
	{
		Vector2 position2 = position + new Vector2(0f, 1080f - scroll).FloorV2();
		foreach (CreditNode credit in credits)
		{
