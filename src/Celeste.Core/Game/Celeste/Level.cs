...
public class Level : Scene, IOverlayHandler
{
	...
	private bool androidCompatibilityCompositorLogged;
	...
	public override void Begin()
	{
		...
		GameplayBuffers.EnsureValid("Level.Begin", forceRecreate: true);
	...
	{
		GameplayBuffers.EnsureValid("Level.HandleGraphicsReset", forceRecreate: true);
		base.HandleGraphicsReset();
	}

	public override void HandleGraphicsCreate()
	{
		GameplayBuffers.EnsureValid("Level.HandleGraphicsCreate", forceRecreate: true);
		base.HandleGraphicsCreate();
	}

	private static bool ShouldUseAndroidCompatibilityCompositor()
	{
		return OperatingSystem.IsAndroid() && (GraphicsBlendCompatibility.UseLegacyBlendStates || AndroidRuntimeTuning.ForceCompatibilityCompositor);
	}

	private static void EnsureGameplayBuffersAvailable(string source)
	{
		GameplayBuffers.EnsureValid(source);
	}
	...
	{
		EnsureGameplayBuffersAvailable("Level.BeforeRender");
	...
	{
		EnsureGameplayBuffersAvailable("Level.Render");
		bool flag = ShouldUseAndroidCompatibilityCompositor();
		bool flag2 = OperatingSystem.IsAndroid() && AndroidRuntimeTuning.DisablePostProcessing;
		bool flag3 = OperatingSystem.IsAndroid() && AndroidRuntimeTuning.DisableBloom;
		if (flag && !androidCompatibilityCompositorLogged)
		{
			androidCompatibilityCompositorLogged = true;
			CelestePathBridge.LogWarn("GPU", "Android compatibility compositor enabled: skipping distortion, bloom, glitch and color grading to prevent gameplay black-screen.");
		}
		...
		if (flag || flag2)
		{
			Draw.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, Engine.FinalBlitSampler, DepthStencilState.Default, RasterizerState.CullNone, null);
			Draw.SpriteBatch.Draw((RenderTarget2D)GameplayBuffers.Gameplay, Vector2.Zero, Color.White);
		...
		Effect effect = (flag ? null : ColorGrade.Effect);
		Draw.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, Engine.FinalBlitSampler, DepthStencilState.Default, RasterizerState.CullNone, effect, matrix);
