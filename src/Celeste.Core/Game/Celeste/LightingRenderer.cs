...
public class LightingRenderer : Renderer
{
	...
	public static readonly BlendState LegacyGradientBlendState = new BlendState
	{
	...
	public static readonly BlendState LegacyOccludeBlendState = new BlendState
	{
		AlphaBlendFunction = BlendFunction.Add,
		ColorBlendFunction = BlendFunction.Add,
	...
	private static bool legacyLightingBypassLogged;

	private bool bypassLightingThisFrame;
	...
	{
		if (GraphicsBlendCompatibility.UseLegacyBlendStates)
		{
			bypassLightingThisFrame = true;
			if (!legacyLightingBypassLogged)
			{
				legacyLightingBypassLogged = true;
				CelestePathBridge.LogWarn("GPU", "Dynamic lighting renderer disabled on Android compatibility mode to prevent black-screen rendering.");
			}

			return;
		}

		bypassLightingThisFrame = false;
	...
	private void ClearDirtyLights(Matrix matrix)
	{
		...
		DrawPrimitiveMaskPass(matrix, ResolveOccludeBlendState(), LegacyOccludeBlendState, "LightingRenderer.ClearDirtyLights");
	}
	...
	private void DrawLightGradients(Matrix matrix)
	{
		...
		DrawPrimitiveMaskPass(matrix, ResolveGradientBlendState(), LegacyGradientBlendState, "LightingRenderer.DrawLightGradients");
	}
	...
	private void DrawLightOccluders(Matrix matrix, Level level)
	{
		...
		DrawPrimitiveMaskPass(matrix, ResolveOccludeBlendState(), LegacyOccludeBlendState, "LightingRenderer.DrawLightOccluders");
	}

	private void DrawPrimitiveMaskPass(Matrix matrix, BlendState preferredBlendState, BlendState legacyBlendState, string source)
	{
		try
		{
			DrawPrimitiveMaskPassCore(matrix, preferredBlendState);
		}
		catch (Exception exception) when (GraphicsBlendCompatibility.TryEnableLegacyBlendFallback(source, exception))
		{
			DrawPrimitiveMaskPassCore(matrix, legacyBlendState);
		}
	}

	private static BlendState ResolveGradientBlendState()
	{
		return GraphicsBlendCompatibility.UseLegacyBlendStates ? LegacyGradientBlendState : GradientBlendState;
	}

	private static BlendState ResolveOccludeBlendState()
	{
		return GraphicsBlendCompatibility.UseLegacyBlendStates ? LegacyOccludeBlendState : OccludeBlendState;
	}

	private void DrawPrimitiveMaskPassCore(Matrix matrix, BlendState blendState)
	{
	...
	{
		Vector2 vector = (edgeA - light).FloorV2();
		Vector2 vector2 = (edgeB - light).FloorV2();
	...
	{
		if (bypassLightingThisFrame || GraphicsBlendCompatibility.UseLegacyBlendStates)
		{
			return;
		}
